...
void print_progress()
{
	...
	printf("FORMAT: (Progress / Time elapsed)\n");
...
int check_for_deadlock()
{
	...
	for (i = 0; i < NUM_PHILS; i++) {
		 ...
		 * library call. Use /proc/self/task/tid/stat
		 */
		 ...
		 *
		 * We want the 14th and 15th field for user time and system
		 * time respectively
		 *
		 * "%*s" saves computation by not actually writing anything,
		 * just consuming the string token
		 */
		for(j = 0; j<13; ++j){
			if(0 > fscanf(statf, "%*s")){
				perror("Error seeking in stat file...");	
			}
		}
		...
		if(0 > fscanf(statf, "%lu", &new_user_time)){
			perror("Error reading user time...");	
		}
//		else{
//			printf("Read new user time %lu\n", new_user_time);	
//		}

		if(0 > fscanf(statf, "%lu", &new_sys_time)){
			perror("Error reading system time...");	
		}
//		else{
//			printf("Read new system time %lu\n", new_sys_time);
//		}
