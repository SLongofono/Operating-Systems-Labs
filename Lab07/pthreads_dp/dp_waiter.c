...
typedef struct {
	...
	pthread_t      	thread;       /* Thread structure for this
					 philosopher */
	int 		wasted_time;  // Track how many times this thread woke up
				      // and was not able to get its chopsticks
} philosopher;
...
static void *dp_thread(void *arg)
{
	...
	while (!Stop) {
		...
		// Grab waiter or wait until waiter available
		pthread_mutex_lock(&waiter);

		// Wait until my chopsticks are free
		...
		*left_chop_available(me) = 0;
		...
		// Unlock my chopsticks
		*right_chop_available(me) = 1;
		*left_chop_available(me) = 1;
		
		// Signal other threads that my chopsticks are free
...
void set_table()
{
	...
	for (i = 0; i < NUM_PHILS; i++) {
		...
		Diners[i].id = i;
		Diners[i].wasted_time = 0;  // Track times woken but not able to eat
	}

	/*
	 * Create each philosopher thread. Note that the address of its
	...
	for (i = 0; i < NUM_PHILS;) {
		...
		for (j = 0; j < 4; j++) {
			...
			sprintf(buf, "%d/%d", 
		  	...
		  	//printf("\nWoke and did nothing %d times...\n", Diners[i].wasted_time);
			i++;
